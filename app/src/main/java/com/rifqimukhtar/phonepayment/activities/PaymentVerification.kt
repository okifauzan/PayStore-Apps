...
class PaymentVerification : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (intent.extras != null){
            ...
            sendRequestPayment = bundle?.getSerializable("sendReqPayment") as SendRequestPayment
    ...
    private fun onClickGroup() {
        ...
        btnPayConfirmOTP.setOnClickListener {
            ...
            if ((textOTP == getOTP) && timerAvailable){
                val paymentModel = SendRequestPayment(1,1,1)
                val paymentCall = ApiClient.getClient()?.create(ApiInteface::class.java)?.sendRequestPayment(paymentModel!!)
